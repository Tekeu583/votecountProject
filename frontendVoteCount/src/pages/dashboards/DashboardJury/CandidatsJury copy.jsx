...
export default function CandidatesList() {
    ...
    const [scrutinFilter, setScrutinFilter] = useState(id ? Number(id) : '');
    ...
    const filteredData = useMemo(() => {
        return candidatesData.filter((c) => {
            ...
            const matchScrutin =
                scrutinFilter === '' || c.electionId === scrutinFilter;
    ...
    }, [search, scrutinFilter, statusFilter, candidatesData]);
    ...
    return (
        <main className="p-4  space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
                <h1 className="text-2xl font-bold ">Liste des candidats {scrutinFilter && (<> <p>de l'election </p> <span className="text-[var(--color-primary)]">{getElectionById(scrutinFilter)} </span></>)} </h1>
            ...
            <div className="bg-white p-4 rounded-xl shadow flex flex-col md:flex-row gap-4">
                ...
                <select
                    value={scrutinFilter}
                    onChange={(e) => {
                        setScrutinFilter(Number(e.target.value));
                        setPage(1);
                    }}
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden">
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        {candidate.status === "EN COURS" ? (
                                            <button
                                                onClick={() => navigate("/jury/candidats/evaluations", { state: { candidate } })}
                                                className="bg-blue-600 text-white px-3 py-1 rounded">
                                                Continuer
                                            </button>
                                        ) : (
                                            <button
                                                onClick={() => navigate("/jury/candidats/evaluations", { state: { candidate } })} className="text-blue-600 font-medium">
                                                Évaluer
                                            </button>
                                        )}
                                    </td>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center p-4 gap-3 text-sm">
                    <span className="text-gray-500">
                        {(page - 1) * perPage + 1} -{" "}
                        {Math.min(page * perPage, filteredData.length)} sur{" "}
                        {filteredData.length}
                    </span>
